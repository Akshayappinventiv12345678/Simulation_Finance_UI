...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.footer-buttons .end-button, .footer-buttons .next-button').forEach(button => {
      button.addEventListener('click', (event) => {
        // Prevent default navigation to handle chart updates before navigation
        event.preventDefault();
  
        // Determine which button was clicked
        if (button.classList.contains('end-button')) {
          // End Simulation button clicked
          // Optionally, perform any specific action here before redirect
          window.location.href = '../../index.html';
        } else if (button.textContent.trim() === 'Adjust Marketing Budget') {
          // Adjust Marketing Budget selected
        ...
        } else if (button.textContent.trim() === 'Introduce New Product Line') {
          // Introduce New Product Line selected
        ...
        // Optionally, update the progress bar or other elements
        const progressBarInner = document.getElementById('progress-bar-inner');
        if (progressBarInner) {
          progressBarInner.style.width = '50%';
        }
  
        // Redirect to the next page after a short delay to allow chart update
        setTimeout(() => {
